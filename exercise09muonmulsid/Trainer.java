...
public class Trainer extends Character {
    ...
    public void sureCapture(Monster m) throws AlreadyCapturedException, FullTeamException{
        ...
        */
        if(team.contains(m)){
            throw new AlreadyCapturedException("Monster is already on the team.\n");
        }
        else if(team.size() >= 6){
            ...
            team.add(m);
            System.out.printf("%s was successfully captured.%n%n", m.getName());}
        }
	...
	if(team.contains(m)){
            team.remove(m);
            System.out.printf("%s was released from the team.%n%n", m.getName());
        }
        else{
